...
- `DOWNTO` 与 `RECORD` 目前已经支持关键字级词法识别
- 但当前版本尚未扩展它们对应的完整语法、语义和代码生成支持
...
- 语法测试：简单程序、无参调用、函数 / 子程序、数组访问、表达式与控制流、`while`、`case` / `break` / `continue`
- AST 测试：子程序参数与函数体构建
- 语义测试：重复声明、未声明标识符、类型不匹配、函数参数个数检查、`break` / `continue` 上下文检查、`while` 条件类型检查
- 符号表测试：作用域、遮蔽、当前作用域查找
- 代码生成对比测试：简单输出、函数调用、字符串输出、`string` 类型变量、`switch` 代码生成
